    ...
    return (
        <div className={classNames("c-menu__item", { "c-menu__item--delete": state === DELETE })}>
            <div className="c-menu__item__info">
                <p className="c-menu__item__name" style={{ paddingLeft: 10 * (+_depID - 1) }}><span className="c-dotted">{"·".repeat(parseInt(_depID))}</span>{ title }</p>
                <span className={classNames("c-menu__item__state", STATE, STATE_CLASSES[state])}>{ state }</span>
                <a className="c-menu__item__link" href={url} target="_blank" rel="noreferrer">{ url }</a>
